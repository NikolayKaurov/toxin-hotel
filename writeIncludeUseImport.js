...
exports.writeIncludeUseImport = function(pages, blocks) {
  let bemEntities = {};
  let useBemEntities = {};
  let blockPath = '';
  let elementPath = '';
  ...
  for (let entity in useBemEntities) {
    if (fs.existsSync(useBemEntities[entity] + slash + entity + '.pug')) {
      ...
      includeThisToPUG = includeThisToPUG + 'include ' + useBemEntities[entity].replace(/^.*blocks/i, '../blocks').replace(/\\/g, '/') + '/' + entity + endLine;
    }
